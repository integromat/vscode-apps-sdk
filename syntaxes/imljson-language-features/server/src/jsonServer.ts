...
namespace SchemaAssociationNotification {
	export const type: NotificationType<ISchemaAssociations | SchemaConfiguration[]> = new NotificationType('imljson/schemaAssociations');
}
...
namespace SchemaContentChangeNotification {
	export const type: NotificationType<string | string[]> = new NotificationType('imljson/schemaContent');
}

namespace ForceValidateRequest {
	export const type: RequestType<string, Diagnostic[], any> = new RequestType('imljson/validate');
}

namespace ForceValidateAllRequest {
	export const type: RequestType<void, void, any> = new RequestType('imljson/validateAll');
}

namespace LanguageStatusRequest {
	export const type: RequestType<string, JSONLanguageStatus, any> = new RequestType('imljson/languageStatus');
}

namespace ValidateContentRequest {
	export const type: RequestType<{ schemaUri: string; content: string }, Diagnostic[], any> = new RequestType('imljson/validateContent');
}
...
namespace DocumentSortingRequest {
	export const type: RequestType<DocumentSortingParams, TextEdit[], any> = new RequestType('imljson/sort');
}
...
export function startServer(connection: Connection, runtime: RuntimeEnvironment) {
		...
		if (dynamicFormatterRegistration) {
			...
			if (enableFormatter) {
				if (!formatterRegistrations) {
					const documentSelector = [{ language: 'imljson' }, { language: 'imljsonc' }];
	...
	async function validateTextDocument(textDocument: TextDocument): Promise<Diagnostic[]> {
		...
		const documentSettings: DocumentLanguageSettings = {
			comments: commentsSeverity ?? (isImljson ? 'ignore' : 'error'),
			trailingCommas: trailingCommasSeverity ?? (isImljson ? 'warning' : 'error'),
