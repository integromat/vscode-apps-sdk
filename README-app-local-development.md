...
**This feature is enabled in pre-release versions only.**
...
- Correct handling components with autogenerated IDs in case of multiple origins.
  - Possible solution 1: Connection will be defined multiple times, each one with ID for specific origins. All connections are linking to same code files.
  - Possible solution 2: The "alternative ID" must be implemented somehow, becase the component has different ID on each origin and there is no logic relation between these IDs.
...
Right-click on `makecomapps.json` and select `Create [component-type] Component`. In this way, you can add new `modules` or `connections`.

*Note: Adding `custom functions`, `RPCs`, and `webhooks` are not implemented yet (but it will be released soon).*

The alternative way is to right-click on an appropriate directory with the list of components. Example: The directory `connetions` has context menu item `Create Connection Component`. Similarly, the directory `modules` has context menu item `Create Module Component`.
...
**Limitation: Multiple origins works with component types, which have user defined IDs (`modules`, `RPCs`, `functions`). But there is the limitation for  `connections`, `webhooks`, where IDs are autogenerated. Therefore for these autogenerated IDs it is not possible to deploy it into multiple origins, because each origin's compoment set uses absolutely different IDs.**
