...
**Limitation: Multiple origins works with component types, which have user defined IDs. But there is the limitation for components like `connection`, where IDs are autogenerated. Therefore for these autogenerated IDs it is not possible to deploy it into multiple origins, because each origin's compoment set uses absolutely different IDs.**
