...
import { downloadComponentCode } from './code-pull-deploy.js';
import { withProgressDialog } from '../utils/vscode-progress-dialog.js';
import { vscodeLibWrapperFactory } from '../services/vscode-lib-wraper/index.js';
...
async function askApiKey(origin: LocalAppOrigin): Promise<string> {
	const { getConfiguration } = await import('../providers/configuration.js'); // Module is limited to IDE (VS Code) only. Dynamic import to avoid issues in CLI environment.
