...
async function bulkDeploy(anyProjectPath: vscode.Uri) {
	...
	await withProgressDialog(
		...
		async (progress, cancellationToken) => {
			...
			const allComponentsSummariesInCloud = await getAllRemoteComponentsSummaries(anyProjectPath, origin, 'remote');
			...
			const newComponentsToCreate = componentAddingRemoving.localOnly.filter((newComponentInMakecomappjson) =>
				codesToDeploy.find(
					(codeToDeploy) =>
						...
						codeToDeploy.componentName === newComponentInMakecomappjson.componentLocalId,
				),
			);

			// TODO REFACTOR FOLLOWING ... or remove
/*			if (newComponentsToCreate.length > 0 || componentAddingRemoving.remoteOnly.length > 0) {
							...
							newComponentsToCreate.length > 0
								? `New ${newComponentsToCreate.length} local components\n` +
								  ...
								  newComponentsToCreate
										...
										.join(', ')
								: '',
							componentAddingRemoving.remoteOnly.length > 0
								? `NOTE: Missing ${componentAddingRemoving.remoteOnly.length} local components:\n` +
								  '(exists in remote Make, but missing in local project):\n' +
								  componentAddingRemoving.remoteOnly
										.map(
											(missingC) =>
			...
			*/
			...
			while ((componentToCreate = newComponentsToCreate.shift())) {
				...
				const newComponentName = await createRemoteAppComponent({
					...
					...componentToCreate,
					componentName: componentToCreate.componentLocalId, // Module, function has local === remote. And for autogenerated names this  property is ignored.
					origin,
				});
			...
			const codesToDeployFinal = codesToDeploy.filter((codePath) => {
				const componentIsMissingInRemote = Boolean(
					componentAddingRemoving.localOnly.find(
						(missingRemoteComponent) =>
							missingRemoteComponent.componentType === codePath.componentType &&
							missingRemoteComponent.componentLocalId === codePath.componentName,
					),
				);
