...
export class CommonCommands {
	static async register(
		...
		vscode.commands.registerCommand('apps-sdk.delete', async function (context) {
			...
			const answer = await vscode.window.showQuickPick(Enum.delete, {
				placeHolder: `Do you really want to delete the ${context.supertype} ${context.bareLabel} (ID ${context.name}) ?`,
			});
			...
			const url = coreUtils.isVersionable(context.apiPath)
				? `${_environment.baseUrl}/${Core.pathDeterminer(_environment.version, '__sdk')}${Core.pathDeterminer(
						_environment.version,
						'app',
				  )}/${app.name}/${app.version}/${Core.pathDeterminer(_environment.version, context.apiPath)}/${
						context.name
				  }`
				: `${_environment.baseUrl}/${Core.pathDeterminer(_environment.version, '__sdk')}${Core.pathDeterminer(
						_environment.version,
						'app',
				  )}/${Core.pathDeterminer(_environment.version, context.apiPath)}/${context.name}`;
			try {
				// Delete the entity
